...
* **Capitolo 1: Tendenze di Mercato**
  ...
  + 1.3 *Composizione Green Portfolio-Mr.Flight*
...
* **Capitolo 3: Cluster Utility**
  + 3.1 *Cluster Utility*
  + 3.2 *Composizione Portfolio Risk objective-Mr.Eco*
  + 3.3 *Composizione Portfolio Risk objective-Mr.Flight*
  + 3.4 *Portfolio Risk objective vs Portfolio Cluster*
  + *Considerazioni*
 
* **APPENDICE**
  + *Composizione Random Portfolio*
  + *Analisi Cluster*
  + *Random Portfolio con asset ordinati*
  + *Riesaminazione scenari*


INTRO {data-icon="fa-list"}
======================================
...
*Ambrosio Francesca*, *Santonicola Gerardo*, *Zappullo Sara*
...
1.1 Analisi preliminare dei dati e calcolo dei rendimenti {data-navmenu="CAPITOLO 1"}
======================================
...
daily_returns <- function(ticker, base_year)
  ...
  # Facciamo in modo da considerare un orizzonte temporale partendo dall'inizio di un anno base (nel nostro caso 2020) 
  #fino all'ultimo giorno di trading disponibile.
  ...
  #Calcoliamo i rendimenti (mensili o giornalieri, basta sostituire daily o monthly in period)
  #consideriamo quelli giornalieri tramite la funzione period return di quantmod.
  data <- periodReturn(stock, period = "daily", type = "arithmetic")
  
  # Facciamo in modo di poter accedere ai dati semplicemente inserendo il Ticker dell'asset (la sigla) 
  #e creando così in automatico un vettore colonna.
...
1.2 Composizione Green Portfolio - Mr. Eco {data-navmenu="CAPITOLO 1"}
=====================================================
...
* GREEN PORTFOLIO <i class="fas fa-solar-panel"></i>

```{r echo=TRUE, message=FALSE, warning=FALSE}
#Comporremo il nostro portafoglio green dai titoli azionari di 25 società che hanno effettuato investimenti 
#per la traslazione al green, e l'indice di riferimento di mercato QCLN come Benchmark.
...
* Si ottiene così l'equivalente di una tabella sulle cui colonne sono riportati i rendimenti dei titoli azionari per ogni giorno (righe) di trading. 
...
green.correlation[upper.tri(green.correlation)] <- NA #sostituiamo i valori del triangolo superiore della matrice 
#con dei missing values
...
plot2 <- ggplot(green.correlation2) +
  ...
  guides(fill = FALSE) +
  xlab("") + ylab("")
#grazie a questa funzione abbiamo ottenuto una mappa di colore interattiva che permette di osservare i valori 
#e la potenza del legame di correlazione tra gli stock del portafoglio.
girafe(ggobj = plot2)
```
...
1.3 Composizione Travel&Food Portfolio - Mr. Flight {data-navmenu="CAPITOLO 1"}
=====================================
...
* FOOD PORTFOLIO <i class="fas fa-utensils"></i> 
...
* TRAVEL PORTFOLIO  <i class="fas fa-plane-departure"></i>  
...
travfood.correlation[upper.tri(travfood.correlation)] <- NA #sostituiamo i valori del triangolo superiore della matrice 
#con dei missing values
...
2.1 Teoria Analisi Cluster {data-navmenu="CAPITOLO 2"}
======================================
...
-	_Distanza euclidea_  
-	_Distanza di Manhattan_  
-	_Distanza di hamming_  
...
2.2 Analisi Cluster - Mr. Eco {data-navmenu="CAPITOLO 2"}
======================================
...
#con la funzione as.dendrogram facciamo in modo di modificare la classe del cluster gerarchico in "dendrogramma".
  ...
  labelColors = c("red", "blue", "green", "orange", "violet", "black", "gray", "brown", "pink", "yellow") 
  #creiamo il vettore dei colori da assegnare al cluster
cluster.green <- cutree(hc.green, 10) #scegliamo il numero dei cluster
  ...
  clusDendro = dendrapply(hcd, colorsLabel) 
  #con dendrapply quindi applichiamo all' oggetto di classe "dendrogram" un colore in base ai cluster tramite la funzione colLab creata in precedenza, che assegna un colore ad ogni livello
```
...
* Nel seguente grafico vedremo come è variato il rendimento del green portfolio rispetto al suo indice azionario di settore.
...
### Green Portfolio Cluster vs. Benchmark
...
2.3 Analisi Cluster - Mr. Flight {data-navmenu="CAPITOLO 2"}
======================================
...
cluster.travrest <- cutree(hc.travrest, 10) #scegliamo il numero dei cluster
  ...
  clusDendro = dendrapply(hcd, colorsLabel) 
  #con dendrapply quindi applichiamo all'oggetto di classe "dendrogram" un colore in base ai cluster 
  #tramite la funzione colLab creata in precedenza, che assegna un colore ad ogni livello
```

### Travel&Food Portfolio
...
### Travel&Food Portfolio Performance vs. Benchmark
...
3.1 Cluster utility {data-navmenu="CAPITOLO 3"}
======================================
...
3.2 Composizione portfolio risk objective - Mr. Eco {data-navmenu="CAPITOLO 3"}
======================================
...
### Green Portfolio Markovitz-Performance vs. Benchmark
...
3.3 Composizione portfolio risk objective - Mr. Flight {data-navmenu="CAPITOLO 3"}
======================================
...
La prudenza insita nel modello di Markovitz gioca a favore di Mr.Flight e nel seguente grafico interattivo risulta ancor più significativa.  Nonostante la tendenza del mercato (descritta dall'indice di riferimento) a sottostimare i settori che compongono il Travel&Food Portfolio, il modello riesce comunque a reggere meglio lo shock temporaneo di domanda e offerta negli ambiti finanziari di ristorazione e di logistica.  Interessante la risposta del portafoglio al secondo lockdown (Ottobre 2020) che resta comunque in attivo nonostante le perdite, a differenza del picco in negativo subito nell'aprile 2020 (shock a cui il mercato era impreparato).  
...
### Travel&Food Portfolio ottimizzato
...
### Travel&Food Portfolio Markovitz Performance vs. Benchmark
...
3.4 Portfolio Risk objective vs Portfolio Cluster {data-navmenu="CAPITOLO 3"}
====================================== 
...
#CON L INSIEME CLUSTER HC
 #in input alla funzione portoflio.returns viene assegnato l'insieme returns.hc, 
#ovvero l'insieme ordinato secondo l'analisi cluster del portafoglio green.
...
#CON L INSIEME CLUSTER HC
 #in input alla funzione portoflio.returns viene assegnato l'insieme returns.hc, 
#ovvero l'insieme ordinato secondo l'analisi cluster del portafoglio trav.rest.
...
### Clustering Performance Portfolio vs Markovitz Model once
...
Considerazioni {data-navmenu="CAPITOLO 3"}
====================================== 

### <i class="far fa-comments"></i> *Considerazioni*  
...
Composizione random portfolio {data-navmenu="APPENDICE"}
====================================== 
...
Dunque, generalemente, un portafoglio è formato da titoli appartenenti a categorie diverse. In seguito ne verrà generato uno composto da titoli scelti dai dataset in modo randomizzato, seguendo quella che può essere la tendenza del mercato e le volontà dell'investitore.
  
</div>  
  
Incrementiamo la lista di ticker a nostra disposizione con titoli appartenenti ad altre categorie. 
  
* TECH PORTFOLIO <i class="fas fa-chalkboard"></i>
...
* PHARMA PORTFOLIO <i class="fas fa-prescription-bottle-alt"></i>
...
Analisi Cluster {data-navmenu="APPENDICE"}
====================================== 
...
Random portfolio con assets ordinati {data-navmenu="APPENDICE"}
====================================== 

Column {.tabset}
---------------------------------------

### *Random portfolio con assets ordinati*
